...
{{-- <script>
    $(document).ready(function() {
        let loading = true;
        const eventsParent = $('#events-list-content');

        if (loading) {
            eventsParent.append(loadingCard(6));
        }

        let retryCount = 0;
        const maxRetries = 3;

        function handleGetEvent(page) {
            $.ajax({
                type: "GET",
                url: "{{ config('app.api_url') }}" + "/api/events?page=" + page,
                headers: {
                ...
                success: function(response) {
                    ...
                    if (response.data.data.length > 0) {
                        $.each(response.data.data, function(index, value) {
                            eventsParent.append(card(index, value));
                        });
